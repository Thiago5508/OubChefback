...
export async function authenticateUser(identifier: AuthIdentifier, password: string) {
  ...
  const token = jwt.sign(
    { _id: user._id, email: user.email, phone:user.phone, name: user.name , responsibility:user.responsibility },
    JWT_SECRET,
    { expiresIn: '12h' } 
  );
